...
# Picture taking frequency:
lastActivatedTime = 0
minWaitingTime = 10 #seconds
...
def detectMotion(isMotion):
    ...
    while (True):
        ...
        if takePicture:
            isMotion[0]=True
            if time.time() - lastCapture > minWaitingTime:
                lastCapture = time.time()
                isMotion[1]=saveImage(cameraSettings, saveWidth, saveHeight, saveQuality, diskSpaceToReserve)
        else:
            isMotion[0]=False
